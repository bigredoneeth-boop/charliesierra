/**
 * AdminDashboardPage
 * Top-level admin overview: security banner, stats cards, recent audit activity,
 * and quick actions. Government-military aesthetic — monospace labels, tight spacing.
 *
 * Layout:
 *  1. Security Banner     — full-width, always visible
 *  2. System Overview     — 5 stat cards (loading skeletons, hover effects)
 *  3. Recent Activity     — enhanced table with time filter + copy buttons
 *  4. Quick Actions       — role-gated action cards
 */
import type { AuditEvent, OrgMembership } from "@/backend";
import { AuditEventType } from "@/backend";
import { AdminLayout } from "@/components/admin/AdminLayout";
import { PrincipalDisplay } from "@/components/admin/PrincipalDisplay";
import { useAuth } from "@/context/auth-context";
import {
  useAdminAuditLog,
  useCheckPolicyExpiry,
  useIsSuperAdmin,
  useMyOrgs,
  useOrgUsers,
  useOrgs,
} from "@/hooks/use-admin";
import { cn } from "@/lib/utils";
import { usePolicyExpiryStore } from "@/stores/policy-expiry-store";
import { useNavigate } from "@tanstack/react-router";
import {
  Building2,
  ChevronRight,
  ClipboardList,
  MessageSquare,
  Settings,
  Shield,
  ShieldCheck,
  UserCheck,
  UserPlus,
  Users,
} from "lucide-react";
import type React from "react";
import { useEffect, useState } from "react";
import { toast } from "sonner";

// ── Constants ─────────────────────────────────────────────────────────────────

/** Time-filter window options for the Recent Activity table */
type TimeFilter = "24h" | "7d" | "all";

const TIME_FILTERS: { value: TimeFilter; label: string }[] = [
  { value: "24h", label: "Last 24h" },
  { value: "7d", label: "Last 7 days" },
  { value: "all", label: "All" },
];

/**
 * EVENT_TYPE_META — mirrors AuditLogTable.tsx exactly.
 * Provides badge label + className for each AuditEventType variant.
 * Keep in sync with types/audit.ts (22 variants total).
 */
type EventTypeMeta = { label: string; className: string };
const EVENT_TYPE_META: Record<AuditEventType, EventTypeMeta> = {
  [AuditEventType.userRegistered]: {
    label: "User Registered",
    className: "bg-blue-500/10 text-blue-700 border-blue-500/30",
  },
  [AuditEventType.messageSent]: {
    label: "Message Sent",
    className: "bg-green-500/10 text-green-700 border-green-500/30",
  },
  [AuditEventType.callInitiated]: {
    label: "Call Initiated",
    className: "bg-purple-500/10 text-purple-700 border-purple-500/30",
  },
  [AuditEventType.memberAdded]: {
    label: "Member Added",
    className: "bg-teal-500/10 text-teal-700 border-teal-500/30",
  },
  [AuditEventType.memberRemoved]: {
    label: "Member Removed",
    className: "bg-orange-500/10 text-orange-700 border-orange-500/30",
  },
  [AuditEventType.memberRoleChanged]: {
    label: "Role Changed",
    className: "bg-teal-500/10 text-teal-700 border-teal-500/30",
  },
  [AuditEventType.memberSuspended]: {
    label: "Member Suspended",
    className: "bg-orange-500/10 text-orange-700 border-orange-500/30",
  },
  [AuditEventType.orgCreated]: {
    label: "Org Created",
    className: "bg-blue-500/10 text-blue-700 border-blue-500/30",
  },
  [AuditEventType.adminAction]: {
    label: "Admin Action",
    className: "bg-red-500/10 text-red-700 border-red-500/30",
  },
  [AuditEventType.userRemoved]: {
    label: "User Removed",
    className: "bg-orange-500/10 text-orange-700 border-orange-500/30",
  },
  [AuditEventType.retentionEnabled]: {
    label: "Retention On",
    className: "bg-yellow-500/10 text-yellow-700 border-yellow-500/30",
  },
  [AuditEventType.retentionDisabled]: {
    label: "Retention Off",
    className: "bg-yellow-500/10 text-yellow-700 border-yellow-500/30",
  },
  [AuditEventType.escrowEnrolled]: {
    label: "Escrow Enrolled",
    className: "bg-indigo-500/10 text-indigo-700 border-indigo-500/30",
  },
  [AuditEventType.escrowRevoked]: {
    label: "Escrow Revoked",
    className: "bg-rose-500/10 text-rose-700 border-rose-500/30",
  },
  [AuditEventType.escrowAccessGranted]: {
    label: "Escrow Access",
    className: "bg-rose-500/10 text-rose-700 border-rose-500/30",
  },
  [AuditEventType.auditLogExported]: {
    label: "Log Exported",
    className: "bg-slate-500/10 text-slate-700 border-slate-500/30",
  },
  [AuditEventType.messageQueueDrained]: {
    label: "Queue Drained",
    className: "bg-amber-500/10 text-amber-700 border-amber-500/30",
  },
  [AuditEventType.priorityMessageSent]: {
    label: "Priority Sent",
    className: "bg-orange-500/10 text-orange-700 border-orange-500/30",
  },
  [AuditEventType.sovereignConfigUpdated]: {
    label: "Sovereign Updated",
    className: "bg-cyan-500/10 text-cyan-700 border-cyan-500/30",
  },
  [AuditEventType.compartmentAssigned]: {
    label: "Compartment Assigned",
    className: "bg-violet-500/10 text-violet-700 border-violet-500/30",
  },
  [AuditEventType.userInvited]: {
    label: "User Invited",
    className: "bg-sky-500/10 text-sky-700 border-sky-500/30",
  },
  [AuditEventType.orgDeleted]: {
    label: "Org Deleted",
    className: "bg-red-500/10 text-red-700 border-red-500/30",
  },
  [AuditEventType.memberReactivated]: {
    label: "Member Reactivated",
    className: "bg-emerald-500/10 text-emerald-700 border-emerald-500/30",
  },
  [AuditEventType.orgSuspended]: {
    label: "Org Suspended",
    className: "bg-orange-500/10 text-orange-700 border-orange-500/30",
  },
  [AuditEventType.groupMemberRemoved]: {
    label: "Group Member Removed",
    className: "bg-rose-500/10 text-rose-700 border-rose-500/30",
  },
  [AuditEventType.orgUpdated]: {
    label: "Org Updated",
    className: "bg-cyan-500/10 text-cyan-700 border-cyan-500/30",
  },
  [AuditEventType.keyRecoveryInitiated]: {
    label: "Recovery Initiated",
    className: "bg-amber-500/10 text-amber-700 border-amber-500/30",
  },
  [AuditEventType.keyRecoveryApproved]: {
    label: "Recovery Approved",
    className: "bg-green-500/10 text-green-700 border-green-500/30",
  },
  [AuditEventType.keyRecoveryRejected]: {
    label: "Recovery Rejected",
    className: "bg-neutral-500/10 text-neutral-600 border-neutral-500/30",
  },
  [AuditEventType.policyReportExported]: {
    label: "Report Exported",
    className: "bg-blue-500/10 text-blue-700 border-blue-500/30",
  },
  [AuditEventType.policyExpiryCheckPerformed]: {
    label: "Expiry Check",
    className: "bg-amber-500/10 text-amber-700 border-amber-500/30",
  },
  [AuditEventType.legalHoldPlaced]: {
    label: "Legal Hold Placed",
    className: "bg-amber-500/10 text-amber-700 border-amber-500/30",
  },
  [AuditEventType.legalHoldRemoved]: {
    label: "Legal Hold Removed",
    className: "bg-amber-500/10 text-amber-700 border-amber-500/30",
  },
  [AuditEventType.retentionPolicyCreated]: {
    label: "Retention Policy Created",
    className: "bg-teal-500/10 text-teal-700 border-teal-500/30",
  },
  [AuditEventType.retentionPolicyUpdated]: {
    label: "Retention Policy Updated",
    className: "bg-teal-500/10 text-teal-700 border-teal-500/30",
  },
  [AuditEventType.platformSettingsUpdated]: {
    label: "Platform Settings Updated",
    className: "bg-cyan-500/10 text-cyan-700 border-cyan-500/30",
  },
  [AuditEventType.orgSettingsUpdated]: {
    label: "Org Settings Updated",
    className: "bg-cyan-500/10 text-cyan-700 border-cyan-500/30",
  },
  [AuditEventType.keyRecoveryCompleted]: {
    label: "Key Recovery Completed",
    className: "bg-amber-500/10 text-amber-700 border-amber-500/30",
  },
  [AuditEventType.keyEscrowEnrolled]: {
    label: "Key Escrow Enrolled",
    className: "bg-blue-500/10 text-blue-700 border-blue-500/30",
  },
};

// ── Helpers ───────────────────────────────────────────────────────────────────

/** Resolve the principal text from a Principal object or string */
function principalText(p: unknown): string {
  if (!p) return "";
  if (typeof (p as { toText?: () => string }).toText === "function") {
    return (p as { toText: () => string }).toText();
  }
  return String(p);
}

/**
 * Compact principal display: first 8 chars + "..." + last 6 chars.
 * E.g. "dzdlk-gu...lu-wqe"
 */
function compactPrincipal(p: string): string {
  if (p.length <= 16) return p;
  return `${p.slice(0, 8)}...${p.slice(-6)}`;
}

/** ISO timestamp for tooltip display */
function isoTimestamp(ts: bigint): string {
  const ms = Number(ts / BigInt(1_000_000));
  return new Date(ms).toISOString();
}

/** Human-readable relative time from bigint nanoseconds */
function relativeTime(ts: bigint): string {
  const ms = Number(ts / BigInt(1_000_000));
  const diffMs = Date.now() - ms;
  const diffSec = Math.floor(diffMs / 1000);
  if (diffSec < 60) return `${diffSec}s ago`;
  const diffMin = Math.floor(diffSec / 60);
  if (diffMin < 60) return `${diffMin}m ago`;
  const diffHr = Math.floor(diffMin / 60);
  if (diffHr < 24) return `${diffHr}h ago`;
  const diffDay = Math.floor(diffHr / 24);
  return `${diffDay}d ago`;
}

/** Filter events to those within the given time window */
function filterByWindow(
  events: AuditEvent[],
  filter: TimeFilter,
): AuditEvent[] {
  if (filter === "all") return events;
  const windowMs =
    filter === "24h" ? 24 * 60 * 60 * 1000 : 7 * 24 * 60 * 60 * 1000;
  const cutoff = BigInt(Date.now() - windowMs) * BigInt(1_000_000);
  return events.filter((e) => e.timestamp >= cutoff);
}

/** Count unique actorPrincipals in the last 24h */
function countActiveUsers(events: AuditEvent[]): number {
  const cutoff = BigInt(Date.now() - 24 * 60 * 60 * 1000) * BigInt(1_000_000);
  const recent = events.filter((e) => e.timestamp >= cutoff);
  const uniq = new Set(recent.map((e) => principalText(e.actorPrincipal)));
  return uniq.size;
}

/** Count messageSent events in the last 7 days */
function countMessages7d(events: AuditEvent[]): number {
  const cutoff =
    BigInt(Date.now() - 7 * 24 * 60 * 60 * 1000) * BigInt(1_000_000);
  return events.filter(
    (e) =>
      e.timestamp >= cutoff &&
      String(e.eventType) === AuditEventType.messageSent,
  ).length;
}

// ── Security Status Card ────────────────────────────────────────────────────
function SecurityStatusCard() {
  const checkTime = new Date().toLocaleString();
  return (
    <div
      className={cn(
        "group rounded-sm border border-green-200 dark:border-green-800",
        "bg-green-50 dark:bg-green-950/30 p-5 shadow-sm",
        "transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 hover:border-green-300 dark:hover:border-green-700",
      )}
      data-ocid="dashboard.stats.security_status"
    >
      <div className="flex items-start justify-between gap-2">
        <p className="font-mono text-[0.6rem] font-semibold uppercase tracking-[0.18em] text-green-700 dark:text-green-400">
          Security Status
        </p>
        <ShieldCheck
          size={28}
          className="shrink-0 text-green-600 dark:text-green-400 transition-colors duration-200 group-hover:text-green-500"
          aria-hidden="true"
        />
      </div>
      <div className="mt-2 flex items-center gap-2">
        <span
          className="h-2 w-2 rounded-full bg-green-500 animate-pulse shrink-0"
          aria-hidden="true"
        />
        <p className="font-mono text-2xl font-bold text-green-700 dark:text-green-300 leading-none tracking-wide">
          SECURE
        </p>
      </div>
      <div className="mt-3 space-y-0.5">
        <p className="font-mono text-[0.55rem] uppercase tracking-widest text-green-600/70 dark:text-green-400/70">
          Last System Check
        </p>
        <p className="font-mono text-[0.6rem] text-green-700 dark:text-green-400 tabular-nums">
          {checkTime}
        </p>
      </div>
      <div className="mt-2 inline-flex items-center gap-1 rounded-sm border border-green-300/60 dark:border-green-700/60 bg-green-100/60 dark:bg-green-900/40 px-1.5 py-0.5">
        <span className="font-mono text-[0.55rem] font-semibold uppercase tracking-widest text-green-700 dark:text-green-400">
          All Clear
        </span>
      </div>
    </div>
  );
}

// ── Skeleton placeholder ──────────────────────────────────────────────────────
function SkeletonCell({ className }: { className?: string }) {
  return (
    <span
      className={cn(
        "inline-block animate-pulse rounded-sm bg-muted",
        className,
      )}
      aria-hidden="true"
    />
  );
}

// ── Security Banner ───────────────────────────────────────────────────────────
/**
 * SecurityBanner — full-width, always visible.
 * Dark background with shield icon communicates high-security context.
 */
function SecurityBanner() {
  return (
    <div
      className="flex items-center gap-3 rounded-sm border border-amber-300 bg-amber-50 px-5 py-3"
      role="banner"
      aria-label="Security environment notice"
      data-ocid="dashboard.security_banner"
    >
      <Shield className="h-4 w-4 shrink-0 text-black/70" aria-hidden="true" />
      <p className="font-mono text-[0.65rem] font-semibold uppercase tracking-[0.2em] text-black">
        HIGH SECURITY ENVIRONMENT —{" "}
        <span className="font-normal">
          All actions are audited and immutable on the Internet Computer.
        </span>
      </p>
    </div>
  );
}

// ── Stat card ─────────────────────────────────────────────────────────────────
interface StatCardProps {
  label: string;
  value: string | number | React.ReactNode;
  icon: React.ElementType;
  loading?: boolean;
  accent?: string; // optional icon color class override
  "data-ocid"?: string;
}

function StatCard({
  label,
  value,
  icon: Icon,
  loading,
  accent,
  ...rest
}: StatCardProps) {
  return (
    <div
      className={cn(
        "group rounded-sm border border-border bg-card p-5 shadow-sm",
        "transition-all duration-200 hover:shadow-md hover:-translate-y-px hover:border-border/80",
      )}
      {...rest}
    >
      <div className="flex items-start justify-between gap-2">
        <p className="font-mono text-[0.6rem] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
          {label}
        </p>
        <Icon
          className={cn(
            "h-3.5 w-3.5 shrink-0 transition-colors duration-200",
            accent ??
              "text-muted-foreground/50 group-hover:text-muted-foreground/70",
          )}
          aria-hidden="true"
        />
      </div>
      <p className="mt-3 font-mono text-4xl font-bold leading-none tabular-nums text-foreground">
        {loading ? <SkeletonCell className="h-8 w-16" /> : value}
      </p>
    </div>
  );
}

// ── Recent Activity table ─────────────────────────────────────────────────────
/**
 * Enhanced RecentActivityTable with:
 *  - Time filter pills (24h / 7d / all)
 *  - Columns: ACTOR, ACTION (badge), TARGET, TIMESTAMP (relative + ISO tooltip), IP
 *  - Max 10 rows; link to full audit page
 *  - Loading skeleton (5 rows)
 *  - Empty state per filter window
 */
function RecentActivityTable({
  events,
  loading,
  error,
  onViewAll,
}: {
  events: AuditEvent[];
  loading: boolean;
  error: string | null;
  onViewAll: () => void;
}) {
  const [timeFilter, setTimeFilter] = useState<TimeFilter>("7d");

  if (error) {
    return (
      <div
        className="rounded-sm border border-destructive/50 bg-destructive/5 px-4 py-3"
        data-ocid="dashboard.audit.error_state"
        role="alert"
      >
        <p className="font-mono text-xs text-destructive">ERROR — {error}</p>
      </div>
    );
  }

  const filtered = filterByWindow(events, timeFilter).slice(0, 10);
  const COLS = ["ACTOR", "ACTION", "TARGET", "TIMESTAMP", "IP"] as const;

  return (
    <div data-ocid="dashboard.audit.section">
      {/* Time filter pills */}
      <div
        className="mb-3 flex items-center justify-between gap-2"
        data-ocid="dashboard.audit.filter_row"
      >
        <fieldset className="flex gap-1.5">
          <legend className="sr-only">Time filter</legend>
          {TIME_FILTERS.map(({ value, label }) => (
            <button
              key={value}
              type="button"
              data-ocid={`dashboard.audit.filter.${value}`}
              onClick={() => setTimeFilter(value)}
              className={cn(
                "rounded-sm border px-2.5 py-1 font-mono text-[0.6rem] uppercase tracking-widest",
                "transition-colors duration-150 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary",
                timeFilter === value
                  ? "border-foreground/40 bg-foreground text-background"
                  : "border-border bg-card text-muted-foreground hover:border-border/80 hover:text-foreground",
              )}
            >
              {label}
            </button>
          ))}
        </fieldset>
        <button
          type="button"
          onClick={onViewAll}
          data-ocid="dashboard.audit.view_all_button"
          className="font-mono text-[0.6rem] uppercase tracking-widest text-muted-foreground hover:text-foreground transition-colors duration-150"
        >
          View all →
        </button>
      </div>

      {/* Table */}
      <div className="overflow-x-auto" data-ocid="dashboard.audit.table">
        <table className="w-full min-w-[600px] border-collapse text-xs">
          <thead>
            <tr className="border-b border-border">
              {COLS.map((h) => (
                <th
                  key={h}
                  className="py-3 pr-4 text-left font-mono text-[0.6rem] font-semibold uppercase tracking-widest text-muted-foreground last:pr-0"
                >
                  {h}
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ["sk1", "sk2", "sk3", "sk4", "sk5"].map((sid, i) => (
                <tr
                  key={sid}
                  className={cn(
                    "border-b border-border/50",
                    i % 2 === 0 ? "bg-muted/15" : "bg-transparent",
                  )}
                >
                  {(["sc0", "sc1", "sc2", "sc3", "sc4"] as const).map(
                    (sid, ci) => (
                      <td key={sid} className="py-2.5 pr-4">
                        <SkeletonCell
                          className={`h-3 ${
                            ["w-28", "w-24", "w-20", "w-16", "w-8"][ci]
                          }`}
                        />
                      </td>
                    ),
                  )}
                </tr>
              ))
            ) : filtered.length === 0 ? (
              <tr>
                <td
                  colSpan={5}
                  className="py-10 text-center font-mono text-xs text-muted-foreground"
                  data-ocid="dashboard.audit.empty_state"
                >
                  No recent activity to display.
                </td>
              </tr>
            ) : (
              filtered.map((ev, i) => {
                const actor = principalText(ev.actorPrincipal);
                const target = ev.targetPrincipal
                  ? principalText(ev.targetPrincipal)
                  : ev.orgId
                    ? String(ev.orgId)
                    : null;
                const meta =
                  EVENT_TYPE_META[String(ev.eventType) as AuditEventType] ??
                  EVENT_TYPE_META[AuditEventType.adminAction];

                return (
                  <tr
                    key={ev.id.toString()}
                    data-ocid={`dashboard.audit.item.${i + 1}`}
                    className={cn(
                      "border-b border-border/40 transition-colors duration-150 hover:bg-muted/40 cursor-default",
                      i % 2 === 0 ? "bg-muted/15" : "bg-transparent",
                    )}
                  >
                    {/* ACTOR */}
                    <td className="py-3 pr-4">
                      <PrincipalDisplay principal={actor} />
                    </td>
                    {/* ACTION */}
                    <td className="py-3 pr-4">
                      <span
                        className={cn(
                          "inline-block rounded-sm border px-1.5 py-0.5",
                          "font-mono text-[0.6rem] font-medium uppercase tracking-wider",
                          meta.className,
                        )}
                      >
                        {meta.label}
                      </span>
                    </td>
                    {/* TARGET */}
                    <td className="py-3 pr-4">
                      {target ? (
                        <PrincipalDisplay principal={target} />
                      ) : (
                        <span className="font-mono text-[0.65rem] text-muted-foreground/40">
                          —
                        </span>
                      )}
                    </td>
                    {/* TIMESTAMP */}
                    <td
                      className="py-3 pr-4 font-mono text-[0.65rem] tabular-nums text-muted-foreground whitespace-nowrap"
                      title={isoTimestamp(ev.timestamp)}
                    >
                      {relativeTime(ev.timestamp)}
                    </td>
                    {/* IP */}
                    <td className="py-3 font-mono text-[0.65rem] text-muted-foreground/40">
                      —
                    </td>
                  </tr>
                );
              })
            )}
          </tbody>
        </table>
      </div>
    </div>
  );
}

// ── Quick Action card ─────────────────────────────────────────────────────────
/**
 * Larger card-style action button with icon, label, description, and chevron.
 * Hover effect: subtle lift + border highlight.
 */
function QuickActionCard({
  label,
  description,
  icon: Icon,
  onClick,
  ocid,
}: {
  label: string;
  description: string;
  icon: React.ElementType;
  onClick: () => void;
  ocid: string;
}) {
  return (
    <button
      type="button"
      data-ocid={ocid}
      onClick={onClick}
      className={cn(
        "group flex items-center gap-3 rounded-sm border border-border bg-card p-4 text-left w-full",
        "transition-all duration-200 hover:shadow-sm hover:-translate-y-px hover:border-foreground/30",
        "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary",
      )}
    >
      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-sm border border-border/60 bg-muted/40 transition-colors duration-200 group-hover:border-foreground/20 group-hover:bg-muted/60">
        <Icon
          className="h-4 w-4 text-muted-foreground group-hover:text-foreground transition-colors duration-200"
          aria-hidden="true"
        />
      </div>
      <div className="min-w-0 flex-1">
        <p className="font-mono text-[0.7rem] font-semibold uppercase tracking-wider text-foreground">
          {label}
        </p>
        <p className="mt-0.5 font-mono text-[0.6rem] text-muted-foreground/70 truncate">
          {description}
        </p>
      </div>
      <ChevronRight
        className="h-3.5 w-3.5 shrink-0 text-muted-foreground/40 transition-transform duration-200 group-hover:translate-x-0.5 group-hover:text-muted-foreground"
        aria-hidden="true"
      />
    </button>
  );
}

// ── Page ──────────────────────────────────────────────────────────────────────
export default function AdminDashboardPage() {
  const navigate = useNavigate();
  const { principal } = useAuth();

  const setExpiryCount = usePolicyExpiryStore((s) => s.setExpiryCount);

  // ── Data queries ────────────────────────────────────────────────────────────
  const { data: isSuperAdmin } = useIsSuperAdmin();
  const { data: expiringPolicies = [] } = useCheckPolicyExpiry();
  const { data: myOrgs } = useMyOrgs();

  // Use first org membership for per-org user count
  const firstOrgId: string | undefined = (
    myOrgs as OrgMembership[] | undefined
  )?.[0]?.orgId;

  const {
    data: orgsData,
    isLoading: orgsLoading,
    error: orgsError,
  } = useOrgs({ limit: BigInt(100), afterOrgId: undefined });

  const { data: orgUsersData, isLoading: usersLoading } = useOrgUsers(
    firstOrgId
      ? { orgId: firstOrgId, limit: BigInt(100), afterUserId: undefined }
      : null,
  );

  // Fetch a larger audit log window to support client-side stats computation
  const {
    data: auditEvents = [],
    isLoading: auditLoading,
    error: auditError,
  } = useAdminAuditLog({ limit: BigInt(200), afterEventId: undefined });

  const typedAuditEvents = auditEvents as AuditEvent[];

  // ── Derived stats ────────────────────────────────────────────────────────────

  // 1. Total Organizations
  const totalOrgsVal: string = orgsLoading
    ? "…"
    : orgsError
      ? "ERR"
      : String(Number(orgsData?.total ?? 0));

  // 2. Total Users (platform-wide) — sum of memberCount across all orgs
  const totalUsersVal: string = orgsLoading
    ? "…"
    : orgsError
      ? "ERR"
      : isSuperAdmin
        ? String(
            (orgsData?.orgs ?? []).reduce(
              (acc, org) => acc + Number(org.memberCount ?? 0),
              0,
            ),
          )
        : // For non-SA: fall back to users in first org
          usersLoading
          ? "…"
          : String(Number(orgUsersData?.total ?? 0));

  // 3. Active Users (last 24h) — unique actorPrincipals in audit log last 24h
  const activeUsersVal: string = auditLoading
    ? "…"
    : String(countActiveUsers(typedAuditEvents));

  // 4. Total Messages (last 7 days) — messageSent events in last 7d
  const messagesVal: string = auditLoading
    ? "…"
    : String(countMessages7d(typedAuditEvents));

  // ── Auth principal display ────────────────────────────────────────────────
  const principalFull = principal?.toText() ?? "";
  const principalCompact = compactPrincipal(principalFull);

  // ── Sync expiry count to global store ────────────────────────────────────────
  const expiryAlertCount = expiringPolicies.length;
  useEffect(() => {
    setExpiryCount(expiryAlertCount);
  }, [expiryAlertCount, setExpiryCount]);

  const [expiryBannerDismissed, setExpiryBannerDismissed] = useState(false);

  // ── Error states ─────────────────────────────────────────────────────────────
  const topError: string | null = orgsError
    ? String((orgsError as Error).message)
    : null;

  const auditErrorMsg: string | null = auditError
    ? String((auditError as Error).message)
    : null;

  return (
    <AdminLayout title="Dashboard">
      {/* ── Security Banner ─────────────────────────────────────────────── */}
      <SecurityBanner />

      {/* ── Policy Expiry Alert Banner ──────────────────────────────────── */}
      {expiryAlertCount > 0 && !expiryBannerDismissed && (
        <div
          className="mt-3 flex items-center justify-between gap-3 rounded-sm border border-amber-400/60 bg-amber-50 px-4 py-2.5"
          role="alert"
          data-ocid="dashboard.policy_expiry_banner"
        >
          <div className="flex items-center gap-2">
            <svg
              className="h-4 w-4 shrink-0 text-amber-600"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
              aria-hidden="true"
            >
              <path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" />
              <line x1="12" y1="9" x2="12" y2="13" />
              <line x1="12" y1="17" x2="12.01" y2="17" />
            </svg>
            <span className="font-mono text-[0.65rem] font-semibold uppercase tracking-widest text-amber-800">
              Policy Expiry Alert:{" "}
              <span className="font-normal">
                {expiryAlertCount} retention{" "}
                {expiryAlertCount === 1 ? "policy" : "policies"} expiring within
                30 days. Review required in{" "}
                <a
                  href="/admin/retention-policies"
                  className="underline hover:text-amber-900 transition-colors"
                  data-ocid="dashboard.policy_expiry_link"
                >
                  Retention Policies
                </a>
                .
              </span>
            </span>
          </div>
          <button
            type="button"
            onClick={() => setExpiryBannerDismissed(true)}
            className="shrink-0 rounded-sm px-2 py-1 font-mono text-[0.6rem] uppercase tracking-widest text-amber-700 hover:bg-amber-100 hover:text-amber-900 transition-colors"
            data-ocid="dashboard.policy_expiry_dismiss_button"
          >
            Dismiss
          </button>
        </div>
      )}

      {/* ── Logged-in user ──────────────────────────────────────────────── */}
      {principalFull && (
        <div
          className="mt-3 flex items-center gap-2"
          data-ocid="dashboard.user_info"
        >
          <span className="font-mono text-[0.6rem] uppercase tracking-widest text-muted-foreground">
            Authenticated as:
          </span>
          <span
            title={principalFull}
            className="font-mono text-[0.65rem] text-foreground cursor-default select-all"
          >
            {principalCompact}
          </span>
          {isSuperAdmin && (
            <span className="rounded-sm border border-red-500/30 bg-red-500/10 px-1.5 py-0.5 font-mono text-[0.55rem] font-semibold uppercase tracking-widest text-red-700">
              Super Admin
            </span>
          )}
        </div>
      )}

      {/* ── Top-level error banner ───────────────────────────────────────── */}
      {topError && (
        <div
          className="mt-4 rounded-sm border border-destructive/50 bg-destructive/5 px-4 py-3"
          data-ocid="dashboard.error_state"
          role="alert"
        >
          <p className="font-mono text-xs font-semibold text-destructive uppercase tracking-wider">
            ERROR — {topError}
          </p>
        </div>
      )}

      <div className="mt-6 space-y-8">
        {/* ── Stats row ─────────────────────────────────────────────────── */}
        <section aria-labelledby="stats-heading">
          <p
            id="stats-heading"
            className="mb-3 font-mono text-[0.6rem] font-semibold uppercase tracking-[0.2em] text-muted-foreground"
          >
            System Overview
          </p>
          {/*
           * 5 cards:
           * - mobile:  2 columns
           * - tablet:  3 columns
           * - desktop: 5 columns
           */}
          <div
            className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5"
            data-ocid="dashboard.stats.section"
          >
            {/* 1. Total Organizations */}
            {isSuperAdmin ? (
              <StatCard
                label="Total Organizations"
                value={totalOrgsVal}
                icon={Building2}
                loading={orgsLoading}
                accent="text-blue-600/70"
                data-ocid="dashboard.stats.total_orgs"
              />
            ) : (
              <StatCard
                label="My Organizations"
                value={String(
                  (myOrgs as OrgMembership[] | undefined)?.length ?? 0,
                )}
                icon={Building2}
                accent="text-blue-600/70"
                data-ocid="dashboard.stats.my_orgs"
              />
            )}

            {/* 2. Total Users (platform-wide) */}
            <StatCard
              label="Total Users"
              value={totalUsersVal}
              icon={Users}
              loading={orgsLoading || (!!firstOrgId && usersLoading)}
              accent="text-indigo-600/70"
              data-ocid="dashboard.stats.total_users"
            />

            {/* 3. Active Users (last 24h) */}
            <StatCard
              label="Active (24h)"
              value={activeUsersVal}
              icon={UserCheck}
              loading={auditLoading}
              accent="text-teal-600/70"
              data-ocid="dashboard.stats.active_users"
            />

            {/* 4. Total Messages (last 7 days) */}
            <StatCard
              label="Messages (7d)"
              value={messagesVal}
              icon={MessageSquare}
              loading={auditLoading}
              accent="text-violet-600/70"
              data-ocid="dashboard.stats.messages_7d"
            />

            {/* 5. Security Status — prominent green card */}
            <SecurityStatusCard />
          </div>
        </section>

        {/* ── Recent Activity ────────────────────────────────────────────── */}
        <section aria-labelledby="activity-heading">
          <p
            id="activity-heading"
            className="mb-3 font-mono text-[0.6rem] font-semibold uppercase tracking-[0.2em] text-muted-foreground"
          >
            Recent Activity
          </p>
          <div className="rounded-sm border border-border bg-card px-4 py-4">
            <RecentActivityTable
              events={typedAuditEvents}
              loading={auditLoading}
              error={auditErrorMsg}
              onViewAll={() => navigate({ to: "/admin/audit-logs" })}
            />
          </div>
        </section>

        {/* ── Quick Actions ──────────────────────────────────────────────── */}
        <section aria-labelledby="actions-heading">
          <p
            id="actions-heading"
            className="mb-3 font-mono text-[0.6rem] font-semibold uppercase tracking-[0.2em] text-muted-foreground"
          >
            Quick Actions
          </p>
          {/*
           * Role-gated quick actions:
           * - Create Org:        SuperAdmin only
           * - Invite User:       OrgAdmin and above (always shown except pure Auditor)
           * - View Audit Logs:   Auditor and above
           * - System Settings:   SuperAdmin only
           */}
          <div
            className="grid grid-cols-1 gap-3 sm:grid-cols-2"
            data-ocid="dashboard.quick_actions.section"
          >
            {/* Create Organization — SuperAdmin only */}
            {isSuperAdmin && (
              <QuickActionCard
                label="Create Organization"
                description="Provision a new tenant or agency org"
                icon={Building2}
                ocid="dashboard.create_org_button"
                onClick={() => navigate({ to: "/admin/organizations" })}
              />
            )}

            {/* Invite User — OrgAdmin and above (everyone with admin access) */}
            <QuickActionCard
              label="Invite User"
              description="Add a user to an organization by principal"
              icon={UserPlus}
              ocid="dashboard.invite_user_button"
              onClick={() => navigate({ to: "/admin/users" })}
            />

            {/* View Full Audit Logs — Auditor and above */}
            <QuickActionCard
              label="View Full Audit Logs"
              description="Browse immutable records of all admin actions"
              icon={ClipboardList}
              ocid="dashboard.view_audit_logs_button"
              onClick={() => navigate({ to: "/admin/audit-logs" })}
            />

            {/* System Settings — SuperAdmin only */}
            {isSuperAdmin && (
              <QuickActionCard
                label="System Settings"
                description="Platform-wide configuration and controls"
                icon={Settings}
                ocid="dashboard.system_settings_button"
                onClick={() => navigate({ to: "/admin/settings" })}
              />
            )}
          </div>
        </section>
      </div>
    </AdminLayout>
  );
}
